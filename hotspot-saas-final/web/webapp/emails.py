...
def send_email(to_email: str, to_name: str, subject: str, html_body: str):
    ...
    try:
        ...
        pass   # Ne jamais planter l'app à cause d'un email
...
def email_subscription_activated(to_email: str, to_name: str, plan_label: str,
    ...
    first = to_name.split()[0] if to_name.split() else to_name
    inner = f"""
    <div style="width:52px;height:52px;background:rgba(34,197,94,.12);border:2px solid rgba(34,197,94,.3);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.4rem;margin-bottom:1.2rem">✅</div>
